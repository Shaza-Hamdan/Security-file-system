...
// AES encryption service registration with optional key/IV
builder.Services.AddScoped<IAES_EncDecService>(provider =>
{
    ...
    // Validate that Key and IV are provided
    if (string.IsNullOrEmpty(key) || string.IsNullOrEmpty(iv))
    {
        throw new InvalidOperationException("AES Key and IV must be provided in configuration.");
    }

    // Create and return the service instance
    return new AES_EncDecService(key, iv);
});
...
//builder.Services.AddScoped<IAES_EncDecService, AES_EncDecService>();
...
async Task CreateFirstAdmin(WebApplication app)
{
    using (var scope = app.Services.CreateScope())
    {
        ...
        var adminUser = new Registration
        {
            UserName = "admin",
